...
# The committed workflow carries a placeholder, not a personal capture URL, so the
# repository can be shared. Build with --local to bake in WEBHOOK_SITE_URL from .env.
WEBHOOK_SITE_PLACEHOLDER = "https://webhook.site/YOUR-UNIQUE-ID"
...
def main():
    ...
    ap.add_argument("--local", action="store_true",
                    help="bake WEBHOOK_SITE_URL from .env into the workflow (for your own import)")
    args = ap.parse_args()
    load_env()
    ...
    ctx = {
        ...
        "webhook_site_url": (os.environ.get("WEBHOOK_SITE_URL", "").rstrip("/") if args.local
                             else WEBHOOK_SITE_PLACEHOLDER),
    }
...
""")
    ...
    # docs/PROMPTS.md: rationale from PROMPTS.src.md + the exact prompt files deployed above.
    doc = (ROOT / "docs" / "PROMPTS.src.md").read_text(encoding="utf-8")
    for step, fname in pnames.items():
        body = (ROOT / "prompts" / f"{fname}.md").read_text(encoding="utf-8").strip()
        doc = doc.replace("{{prompt:" + step + "}}", body).replace("{{version:" + step + "}}", fname)
    doc = "<!-- GENERATED by scripts/build_workflow.py from docs/PROMPTS.src.md - do not edit -->\n" + doc
    (ROOT / "docs" / "PROMPTS.md").write_text(doc, encoding="utf-8")
    where = ("from .env" if args.local and ctx["webhook_site_url"] else
             "placeholder - rebuild with --local before importing" if not args.local else
             "EMPTY - publish step will be skipped")
    print(f"wrote {out.relative_to(ROOT)}  ({len(nodes)} nodes, prompts {pnames}, "
          f"webhook_site_url {where})")
